  ...
  return (
    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
      {linkedin_recommendations.map((rec, idx) => (
        <motion.div key={idx} className="bg-gray-100 dark:bg-gray-800 rounded-xl shadow-md outline-1 outline-gray-700 transition-transform duration-300 hover:scale-102 p-6"
                whileHover={{ scale: 1.05 }}
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                transition={{ duration: 0.8 }}>
          <img src={rec.avatar} alt={`${rec.name}'s avatar`} className="avatar w-16 h-16 rounded-full mx-auto" />
          <h3 className="text-xl font-semibold mt-4 text-center">{rec.name}</h3>
          <h4 className="text-md font-medium mt-2 text-center text-gray-600 dark:text-gray-300">{rec.title}</h4>
          <p className="text-sm mt-2 text-center text-gray-500 dark:text-gray-400">{rec.sub}</p>
          <p className="text-sm mt-4 text-justify text-gray-600 dark:text-gray-300">{rec.recommendation}</p>
        </motion.div>
      ))}
    </div>
  );
